...
function synthesizeCaseDetailedNarrative(c: BatchCaseItem): {
  narrative: string;
  statusLabel: 'Positive' | 'Negative' | 'Hold';
  statusColor: string;
  ...
  let statusLabel: 'Positive' | 'Negative' | 'Hold' = 'Positive';
...
export async function generateConsolidatedRcuPdf(
  reportTitle: string,
  dateRangeStr: string,
  cases: BatchCaseItem[]
): Promise<Buffer> {
