...
export function generateSearchCandidates(rawAddress: string): string[] {
  ...
  if (tokens.length >= 3) candidates.push(tokens.slice(-3).join(', '));
...
export function findKnownRegion(rawAddress: string): { lat: number; lng: number } | null {
...
function classifyAccuracy(
  ...
  if (houseLike.includes(type)) return 'house';
  if (cityLike.includes(type)) return 'city';
  if (type === 'road' || type === 'street' || type === 'pedestrian' || type === 'footway') return 'street';
  if (address && typeof address === 'object') {
    if (address.house_number || address.building || address.shop) return 'house';
...
export async function geocodeAddress(
  ...
  for (const query of generateSearchCandidates(rawAddress)) {
    ...
    if (r) {
      const result: GeocodeResult = { ...r, source: 'osm' };
