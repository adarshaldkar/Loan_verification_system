...
exports.generateRcuDocxReport = generateRcuDocxReport;
...
function synthesizeResidenceNarrative(data, customer) {
...
function synthesizeBusinessNarrative(data, customer) {
...
function synthesizeResiCumBusinessNarrative(data, customer) {
    const p = data || {};
    ...
    if (p.remarks)
        narrative += `Remarks: ${p.remarks}.`;
    return narrative;
}
function synthesizeOfficePayslipNarrative(data, customer) {
    const p = data || {};
...
function synthesizeAgricultureNarrative(data, customer) {
    const p = data || {};
...
function getVerificationNarrative(caseData) {
    let profileData = {};
    try {
...
function createHeaderCell(text, widthPercent = 35) {
    return new docx_1.TableCell({
        width: { size: widthPercent, type: docx_1.WidthType.PERCENTAGE },
        shading: { type: docx_1.ShadingType.CLEAR, fill: 'F1F5F9' },
...
function createValueCell(text, widthPercent = 65, bold = false, color = '1E293B') {
    return new docx_1.TableCell({
        width: { size: widthPercent, type: docx_1.WidthType.PERCENTAGE },
        borders: CELL_BORDERS,
        verticalAlign: docx_1.VerticalAlign.CENTER,
...
function createTableRow(label, value, boldValue = false, colorValue = '1E293B') {
...
async function generateRcuDocxReport(caseData) {
    ...
    const imageElements = [];
    imageElements.push(new docx_1.Paragraph({
    ...
    if (caseData.media && caseData.media.length > 0) {
        ...
        if (fetchedImages.length > 0) {
            ...
            for (let i = 0; i < fetchedImages.length; i += 2) {
                ...
                if (second) {
                    runs.push(new docx_1.TextRun({ text: '    ' })); // spacing between images
                    runs.push(new docx_1.ImageRun({
                        data: second.buffer,
                        type: 'jpg',
                        transformation: { width: 260, height: 195 },
                    }));
        ...
        sections: [
            {
                ...
                children: [
                    ...
                    new docx_1.Paragraph({ spacing: { before: 120, after: 0 } }),
